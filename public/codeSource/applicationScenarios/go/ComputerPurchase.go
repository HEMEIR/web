...
func (s *ComputerPurchase) Init(ctx contractapi.TransactionContextInterface) error {
	Buyer := Person{Name: "Boyue Kindergarten, Jianghan District, Wuhan City", MSPID: 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4}
	...
	Seller := Person{Name: "Wuhan Dizhihou Trading Co., Ltd.", MSPID: 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4}
	...
	Service := token{Name: "computer", ID: "M43Y", Number: 1, Unit_price: 4950, Price: 4950}
...
// Function: operation1
func (s *ComputerPurchase) operation1(ctx contractapi.TransactionContextInterface) error {
...
// Function: operation2
func (s *ComputerPurchase) operation2(ctx contractapi.TransactionContextInterface) error {
	// Update contract state
...
func (s *ComputerPurchase) cndition1(ctx contractapi.TransactionContextInterface) bool {
	if isDone(ctx, "rule2", s) && !isTime(s.functionFinishTime["rule2"]+60*60*24*3) {
		return true
	}
	return false
}
func (s *ComputerPurchase) condition2(ctx contractapi.TransactionContextInterface) bool {
	if isDone(ctx, "rule2_2", s) && !isTime(s.functionFinishTime["rule2_2"]+60*60*24*3) {
		return true
	}
	return false
}
func (s *ComputerPurchase) rule1(_place string, ctx contractapi.TransactionContextInterface) {
	if onlyState(s.contractState) && !s.functionStatus["rule1"] {
		s.contractMessage.Place = _place
		changeRule("rule1", s)
	}
}
func (s *ComputerPurchase) rule2(ctx contractapi.TransactionContextInterface) {
...
func (s *ComputerPurchase) rule2_2(_dayNum int64, ctx contractapi.TransactionContextInterface) {
...
func (s *ComputerPurchase) rule2_3(ctx contractapi.TransactionContextInterface) {
...
func (s *ComputerPurchase) rule3(_checkResult bool, ctx contractapi.TransactionContextInterface) {
	if onlyState(s.contractState) && !s.functionStatus["rule3"] {
		s.contractMessage.CheckResult = _checkResult
		if s.cndition1(ctx) || s.condition2(ctx) {
...
func (s *ComputerPurchase) rule4(ctx contractapi.TransactionContextInterface) {
	if onlyState(s.contractState) && !s.functionStatus["rule4"] {
		if s.cndition1(ctx) || s.condition2(ctx) && !isDone(ctx, "rule3", s) {
			SellerBytes, _ := ctx.GetStub().GetState("Seller")
			// 将获取到的 token 数据反序列化为 token 结构体
			var Seller Person
			json.Unmarshal(SellerBytes, &Seller)
...
func (s *ComputerPurchase) rule5(_faultName string, _amount float64, ctx contractapi.TransactionContextInterface) {
...
func (s *ComputerPurchase) rule6(ctx contractapi.TransactionContextInterface) {
...
func (s *ComputerPurchase) rule7(ctx contractapi.TransactionContextInterface) {
...
// 检查某个功能是否已经执行
func isDone(ctx contractapi.TransactionContextInterface, functionName string, s *ComputerPurchase) bool {
...
func changeRule(expressionName string, s *ComputerPurchase) {
	// 更新 functionStatus
